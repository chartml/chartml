...
//!
//! Walks the ChartElement tree recursively and emits SVG XML. Lives in
//! `chartml-core` (alongside `ChartElement` itself) so the three-stage
//! pipeline's `render_prepared_to_svg` can produce a `String` without
//! adding a circular dependency on `chartml-render`. `chartml-render`
//! re-exports `element_to_svg` from here for back-compat.
//!
//! Follows the same pattern as `chartml-leptos/src/element.rs` but
//! produces a string instead of Leptos views.

use crate::element::{ChartElement, ElementData};
use std::fmt::Write;
...
mod tests {
    ...
    fn group_with_transform() {
        ...
        let svg = element_to_svg(&element, 800.0, 400.0);
        // A Group is a non-`<svg>` root, so `element_to_svg` wraps it in an
        // `<svg>` envelope + `<foreignObject>`; the group's translate transform
        // must survive into the wrapped output.
